...
void CudaSolverCenter::Init(){
    ...
    CHECK_CUDA(cudaHostAlloc(&tasks_problem_, sizeof(Problem) * CUDA_MAX_TASKS, cudaHostAllocDefault));
    ...
    for(int i = 0; i < CUDA_MAX_TASKS; ++i) {
        ...
        tasks_problem_[i] = problem;  // 复制到分配的内存中
    }
}
...
void CudaSolverCenter::GenerateSolution(int task_id){
    ...
    diff_evolve_solvers_[task_id].InitSolver(gpu_device_, rnd_manager_.get(), &tasks_problem_[task_id], &tasks_best_sol_[task_id], &tasks_potential_sol_[task_id]);
